...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `/home/gregoire/.config/tldraw/server.json`.
...
```bash
PORT=$(jq -r .port '/home/gregoire/.config/tldraw/server.json'); TOKEN=$(jq -r .token '/home/gregoire/.config/tldraw/server.json')
